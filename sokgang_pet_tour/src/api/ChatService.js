...
const apiClient = axios.create({
    ...
    : `${import.meta.env.VITE_DEPLOY_BACKEND_URL}/api/v1/chat`,
...
export const getChatRoomMessageService = async (roomId) => {

    try{
        const chatRoomResponse = await apiClient.get(`/messages/${roomId}`);
...
export const clearUnreadMessageService = async (roomId) => {

    try{
        const chatRoomResponse = await apiClient.patch(`/count/${roomId}`);
        return await chatRoomResponse.data;
    }catch(error){
        if (error.response){
            return error.response.data;
