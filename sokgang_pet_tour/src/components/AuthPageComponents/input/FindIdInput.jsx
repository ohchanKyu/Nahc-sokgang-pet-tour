...
const FindIdInput = (props) => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (Object.values(newErrors).every((err) => err === "")) {
            ...
            const findIdResponseData = await props.findIdHandler(formData.name.value,formData.email.value);
                        ...
                        >
                            ...
                            <ul className={classes.userId_container}>
                                {resultMessage.userId.map((id, index) => {
                                    ...
                                    const visibleText = id.substring(0, visibleLength);
                                    const maskedText = '*'.repeat(length - visibleLength);

                                    return (
                                    <li key={index} className={classes.userId_item}>
                                        # {visibleText}
                                        <span className={classes.maskedText}>{maskedText}</span>
                                    </li>
                                    );
                                })}
                            </ul>
