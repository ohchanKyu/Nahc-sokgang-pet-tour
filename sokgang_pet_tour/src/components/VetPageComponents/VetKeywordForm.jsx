...
import { getVetPlacesByKeywordService } from '../../api/VetService';
import classes from '../TourPageComponents/KeywordForm.module.css';
...
const VetKeywordForm = ({ onSelect }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const res = await getVetPlacesByKeywordService(q);
        if (res?.success) {
          const conv = (res.data || []);
          setResults(conv);
              ...
              {displayed.map((r, idx) => (
                ...
                >
                  ...
                  <div className={classes.itemBody}>
                    <div className={classes.itemTitle}>{r.placeName}</div>
                    <div className={classes.itemMeta}>
                      <span>{r.category === 'HOSPITAL' ? '동물병원' : '동물약국'}</span>
