...
const VerifyCodeInput = (props) => {
    ...
    const sendEmailHandler = async (e) => {
        ...
        if (errors === ""){
            ...
            const sendResponseData = await sendVerifyCodeService(formData.userId.value);
            if (sendResponseData.success){
                setAuthMailToken(sendResponseData.data);
                setIsEmailSend(true);
            }else{
    ...
    const verifyCodeHandler = async (e) => {
        ...
        if (errors === ""){
            setChecking((prev) => ({ ...prev, ["formData"]: true }));
            const verifyCode = formData.verifyCode.value.trim();
            const verifyResponseData = await isVerifyCodeService(authMailToken, verifyCode);
